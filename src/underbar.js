(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allTrue, item) {
      return (allTrue && (iterator(item) ? true : false));
    }, true);
  };
  ...
  _.some = function(collection, iterator) {
    ...
    // call every() to see if all items fail, then return opposite truth value
    var newIter;
    if (iterator === undefined)
      newIter = function(val) { return (val ? false : true); };
    else
      newIter = function(val) { return (iterator(val) ? false : true); };
    
    return !(_.every(collection, newIter));
  };
  ...
  _.extend = function(obj) {
    _.each(arguments, function(argObj, idx){
      if (idx === 0) return;
      else {
  ...
  _.defaults = function(obj) {
    // *can this be rewritten using extend()?
    _.each(arguments, function(argObj, idx){
      if (idx === 0) return;
      else {
        _.each(argObj, function(val, key){
